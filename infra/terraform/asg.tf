...
resource "aws_autoscaling_group" "webapp_asg" {
  ...
  force_delete          = true
  launch_configuration  = aws_launch_configuration.webapp_lc[0].id
...
resource "aws_autoscaling_policy" "scale_up" {
  ...
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.webapp_asg[count.index].name
}
...
resource "aws_cloudwatch_metric_alarm" "scale_up_alarm" {
  ...
  dimensions = {
    AutoScalingGroupName = aws_autoscaling_group.webapp_asg[count.index].name
  }
